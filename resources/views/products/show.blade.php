...
@section('title', ($produk->meta_title ?: $produk->nama_produk) . ' | Tangga Mas Scaffolding')

@section('meta')
    @php
        $seoTitle       = $produk->meta_title ?: $produk->nama_produk;
        $seoDesc        = $produk->meta_description ?: \Illuminate\Support\Str::limit(strip_tags($produk->spesifikasi ?: $produk->deskripsi), 160);
        $seoKeywords    = $produk->meta_keywords ?: 'Jual Scaffolding, Supplier Scaffolding, ' . $produk->nama_produk . ', Tangga Mas';
        $seoAuthor      = $produk->meta_author ?: 'PT. Tangga Mas Jaya Makmur';
        $currentUrl     = request()->url();
        $imageUrl       = $produk->gambar ? asset('images/products/' . $produk->gambar) : asset('images/logotm.png');
    @endphp
    ...
    <meta property="product:price:amount" content="{{ (int)$produk->harga }}">
...
<style>
    /* Styling khusus rendering HTML CKEditor pada detail produk */
    .desc-content ul { list-style-type: disc !important; padding-left: 1.25rem !important; margin-top: 0.25rem !important; margin-bottom: 0.5rem !important; }
    .desc-content ol { list-style-type: decimal !important; padding-left: 1.25rem !important; margin-top: 0.25rem !important; margin-bottom: 0.5rem !important; }
    .desc-content p { margin-bottom: 0.35rem !important; }
    .desc-content u { text-decoration: underline !important; }
    .desc-content strong, .desc-content b { font-weight: 700 !important; }
    .desc-content em, .desc-content i { font-style: italic !important; }
</style>
...
<div class="container mx-auto px-4 py-6 md:py-10 max-w-6xl">
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-0 bg-white rounded-xl border border-gray-150 shadow-sm items-start overflow-hidden">
        ...
        <!-- AREA GAMBAR UTAMA & CAROUSEL THUMBNAIL -->
        <div class="md:col-span-5 w-full md:sticky md:top-28 p-4 md:p-6 space-y-3">
            ...
            <div class="bg-brand-gray-bg aspect-square flex items-center justify-center overflow-hidden relative shadow-inner rounded-xl border border-gray-200">
                <img id="main-product-image" 
                     src="{{ $produk->gambar ? asset('images/products/' . $produk->gambar) : asset('images/placeholder.png') }}" 
            ...
            <!-- CAROUSEL THUMBNAIL BARIS BAWAH -->
            @if($produk->gambar || ($produk->galeri && $produk->galeri->count() > 0))
                <div class="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none" id="thumbnailList">
                    @if($produk->gambar)
                        <button type="button" 
                                onclick="changeMainImage('{{ asset('images/products/' . $produk->gambar) }}', this)"
                    ...
                    @if($produk->galeri && $produk->galeri->count() > 0)
                        @foreach($produk->galeri as $galeri)
                            <button type="button" 
                                    onclick="changeMainImage('{{ asset('images/products/' . $galeri->foto) }}', this)"
        ...
        <div class="md:col-span-7 flex flex-col justify-between min-h-full p-4 md:p-6 py-4 md:py-6">
            <div>
                ...
                <div class="flex items-baseline gap-3 mb-5 border-b border-gray-150 pb-4">
                    <span id="display-harga" class="text-xl md:text-2xl lg:text-3xl font-extrabold text-brand-price">
                ...
                <!-- DESKRIPSI PRODUK (HTML FORMATTED READY) -->
                <div class="mb-6 border-t border-gray-100 pt-4">
                    <h3 class="text-xs font-bold text-gray-800 mb-1.5">Deskripsi Produk:</h3>
                    <div class="relative">
                        <div id="descText" class="text-xs md:text-sm text-gray-600 leading-relaxed desc-content line-clamp-3 transition-all duration-300">
                        ...
                        <button type="button" 
                                ...
                                class="hidden text-xs font-bold text-brand-green hover:text-brand-green-dark mt-1.5 flex items-center gap-1 cursor-pointer focus:outline-none">
                            <span id="toggleDescText">Lihat Selengkapnya</span>
                            <i id="toggleDescIcon" class="fa-solid fa-chevron-down text-[10px]"></i>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3 pt-4 border-t border-gray-150">
                ...
                @else
                    <div class="space-y-2.5">
                        ...
                        <a href="{{ $linkWA }}" 
                           target="_blank" 
                           onclick="trackGAEvent('click_order_whatsapp', { item_id: '{{ $produk->id }}', item_name: '{{ $produk->nama_produk }}', item_category: '{{ $produk->kategori }}', price: {{ (int)$produk->harga }} })"
...
<script>
    ...
    function toggleDescription() {
        ...
        const toggleText = document.getElementById('toggleDescText');
        const toggleIcon = document.getElementById('toggleDescIcon');
    ...
    function addToCart(produkId) {
        ...
        trackGAEvent('add_to_cart', {
            item_id: produkId,
            item_name: '{{ $produk->nama_produk }}',
            item_category: '{{ $produk->kategori }}',
            price: {{ (int)$produk->harga }},
            quantity: payload.quantity,
            variant: payload.ukuran_varian || 'Standard'
        });
